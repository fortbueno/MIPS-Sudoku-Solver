...
# Machine Problem -- Sudoku Solver 4x4
...
take_input:						#take_input reads a 4-digit integer, and stores each digit of this integer to the grid via arithmetic operations
	li $t1, 1000					#set $t1 = 1000
	bge $t0, 4, return				#branch to return if $t0 >= 4
...
check_vacant:
	...
	bgt $s0, 60, return_success			#branch to return_success if %s0 > 60 (the offset has passed the end of the grid)
...
next_guess:
	...
	ble $s4, 4, if_vacant				#branch to if_vacant if $s4 is less than or equal to 4
	sw $zero, grid + 0($s0)				#set the value of the cell back to zero
	li $v0, 1					#set $v0 = 1
...
check_rowcol:
	...
	mul $t2, $a2, 16			#set $t2 = $a2 * 16; $t2 is the offset of the starting cell in the row
...
check_row:
	...
	blt $t1, 4, check_row			#branch to check_row if $t1 < 4
	...
	addi $t3, $t2, 48			#set $t3 = $t2 + 48; we will use this as a bound to check the number of elements we have checked in the column
...
check_col:
	...
	addi $t2, $t2, 16			#else, set $t2 = $t2 + 16; add 16 to the offset to access the next cell in the column
	...
	div $t2, $a2, 2				#set $t2 = $a2 / 2
	div $t3, $a3, 2				#set $t3 = $a3 / 2
	mul $t2, $t2, 32			#set $t2 = $t2 * 32
	mul $t3, $t3, 8				#set $t3 = $t3 * 8
...
check_box:
	...
	bge $t2, 2, check_box_col		#branch to check_box_col if $t2 is greater than or equal to 2
...
check_box_col:
	addi $t3, $t3, 12			#set $t3 = $t3 + 12; get the offset of the element in the bottom left of the box
	li $t2, 0				#set $t2 = 0
	addi $t1, $t1, 1			#set $t1 = $t1 + 1
	blt $t1, 2, check_box			#branch to check_box if $t1 is less than 2
...
print_grid:
	bgt $t0, 60, terminate_program		#branch to terminate_program if $t0 > 60, this means that the offset has ezceeded the grid's bounds
	beq $t1, 4, print_new_line		#else, branch to print_new_line if $t1 = 4, this means that 4 elements in a row has been printed and thus we move on to the next row
